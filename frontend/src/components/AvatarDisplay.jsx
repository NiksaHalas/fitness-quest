...
const AvatarDisplay = ({ level }) => {
  ...
  const getAvatarSvg = (currentLevel) => {
    const baseColor = theme.palette.primary.main; // Osnovna boja avatara
    const accentColor = theme.palette.secondary.main; // Akcentna boja
    ...
    switch (currentLevel) {
      ...
      case 2:
        return (
          <svg width="100%" height="100%" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="50" cy="50" r="40" fill={baseColor} />
            <circle cx="50" cy="40" r="18" fill="white" /> {/* Veća glava */}
            <rect x="30" y="60" width="40" height="20" rx="10" fill="white" /> {/* Veće telo */}
            <circle cx="35" cy="35" r="5" fill={accentColor} /> {/* Mali detalj */}
          </svg>
        );
      case 3:
        return (
          <svg width="100%" height="100%" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="50" cy="50" r="45" fill={baseColor} /> {/* Još veći */}
            ...
            <circle cx="70" cy="30" r="6" fill={accentColor} /> {/* Dva detalja */}
          </svg>
        );
      case 4:
        return (
          <svg width="100%" height="100%" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="5" y="5" width="90" height="90" rx="20" fill={baseColor} /> {/* Kvadratni oblik */}
            <circle cx="50" cy="40" r="22" fill="white" />
            <rect x="20" y="60" width="60" height="30" rx="15" fill="white" />
            <path d="M40 30 L50 20 L60 30" stroke={accentColor} strokeWidth="3" fill="none" /> {/* Ševron */}
      ...
      case 5:
        return (
          <svg width="100%" height="100%" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <polygon points="50 5, 95 40, 80 95, 20 95, 5 40" fill={baseColor} /> {/* Petougaoni oblik (zvezda) */}
            ...
            <path d="M20 50 L80 50" stroke={accentColor} strokeWidth="4" fill="none" /> {/* Dodatna linija */}
  ...
  return (
    <Box sx={{
      ...
      {getAvatarSvg(level)} {/* Prikazujemo SVG */}
    </Box>
  );
};
